...
const QUESTIONS = {
  ...
  povijest: [
    ...
    {q:"Tko je objavio jedini priručnik za učenje bosančice?", o:["Matija Divković","Ivan Berčić (1860.)","Ćiro Truhelka","Benedikta Zelić-Bučan"], a:1, x:"Ivan Berčić objavio je 1860. <i>Bukvar staroslovenskoga jezika glagolskimi pismeni za čitanje crkvenih knjig</i> — jedini dosad objavljen priručnik za učenje bosančice."},
    ...
    {q:"Zbog čega je bosančica s vremenom počela nestajati?", o:["Zbog zabrane pape","Zbog ukidanja sjemeništa u Priku (1821.), zahtjeva austrijskih vlasti za latinicom i otvaranja pučkih škola","Zbog dolaska Turaka","Zbog izuma tiska"], a:1, x:"Najteži udarac bosančici zadan je ukidanjem sjemeništa u Priku 1821., zahtjevom austrijskih vlasti da se u matičnim knjigama koristi latinica, te otvaranjem pučkih škola s latinicom i novijom ćirilicom."},
    ...
    {q:"Na čijem su dvoru pisana ćirilična diplomatska pisma duboko u unutrašnjosti?", o:["Kralja Zvonimira","Bana Kulina","Ugarsko-hrvatskoga kralja Matijaša Korvina","Kralja Tomislava"], a:2, x:"Diplomatska prepiska ćirilicom vodila se i na dvoru ugarsko-hrvatskoga kralja Matijaša Korvina. Mnogi plemići 16. st. koristili su ćirilicu, npr. Nikola Jurišić, branitelj Kisega."}
  ],
  ...
  abeceda: [
    ...
    {q:"Kako Ivan Berčić dijeli bosančicu u svom Bukvaru (1860.)?", o:["Na staru i novu","Na tri geografske inačice","Na glagoljsku i ćiriličnu","Na rukopisnu i tiskanu"], a:1, x:"Berčić je bosančicu, koju naziva <i>bosanska azbukva</i>, podijelio na tri geografske inačice i naveo tiskane i rukopisne oblike slova i brojeva."},
  ...
  spomenici: [
    ...
    {q:"Koje je godine napisana Povelja Kulina bana?", o:["1180.","1184.","1189.","1250."], a:2, x:"<b>Povelja Kulina bana</b> iz 1189. pergamentni je dokument koji svjedoči o postojanju dubrovačke slavenske kancelarije već krajem 12. stoljeća."},
    ...
    {q:"Za koga je pisan Hvalov zbornik?", o:["Za kralja Zvonimira","Za bana Kulina","Za hercega Hrvoja Vukčića Hrvatinića","Za kralja Tomislava"], a:2, x:"<b>Hvalov zbornik</b> (~1404.) iluminirani je rukopis pisan za hercega Hrvoja Vukčića Hrvatinića, vjerojatno u njegovoj rezidenciji u Omišu."},
    {q:"Koja je prva tiskana knjiga bosančicom?", o:["Nauk karstianski","Libro od mnozijeh razloga","Ofičje Blažene Djeve Marije (Venecija, 1512.)","Poljički statut"], a:2, x:"Prvo hrvatskoćirilično tiskano izdanje: <b>Ofičje Blažene Djeve Marije</b>, Venecija 1512."},
    {q:"Koji je pravni spomenik iz 1440. pisan bosančicom?", o:["Vinodolski zakon","Poljički statut","Kulinova povelja","Statut Dubrovnika"], a:1, x:"<b>Poljički statut</b> (1440.) reprezentativan je pravni spomenik. Poljičani su u njemu ćirilicu nazivali glagoljicom."},
    ...
    {q:"Tko je autor <i>Nauka karstianskoga</i> (1611.)?", o:["Ivan Bandulavić","Pavao Posilović","Matija Divković","Franjo Rački"], a:2, x:"<b>Matija Divković</b>, bosanski franjevac, autor je <i>Nauka karstianskoga za narod slovinski</i> (Venecija, 1611.)."},
    {q:"Na kojem se glagoljskom spomeniku nalaze i ćirilična slova?", o:["Bašćanska ploča (~1100.)","Povaljski prag","Humačka ploča","Povaljska listina"], a:0, x:"Na glagoljskoj <b>Bašćanskoj ploči</b> (~1100.) pojavljuju se i ćirilična slova — rani suživot dvaju pisama na hrvatskome tlu."},
...
function startQuiz(topic) {
  ...
  shuffledQuestions = topic === 'mix'
    ...
    : QUESTIONS[topic].map((q,i) => ({...q, topic, origIdx:i}));
...
function renderQuestion() {
  ...
  const icons = {povijest:'📜', abeceda:'🔡', spomenici:'📜', nazivi:'📛'};
  const card = document.getElementById('questionCard');
  card.className = 'q-card topic-' + item.topic;
  card.innerHTML = `<div class="inner">
    <div class="q-num">${icons[item.topic]||''} ${item.topic.charAt(0).toUpperCase()+item.topic.slice(1)} · ${currentIndex+1}/${shuffledQuestions.length}</div>
    <div class="q-text">${item.q}</div>
    <div class="opts">${item.o.map((opt,oi)=>`<div class="opt" onclick="selectAnswer(${oi})"><span class="letter">${'ABCD'[oi]}.</span><span>${opt}</span></div>`).join('')}</div>
...
function selectAnswer(chosen) {
  ...
  if (chosen === item.a) {
    ...
    fb.innerHTML = '✅ <b>Točno!</b> ' + item.o[item.a] + '<span class="explanation">' + item.x + '</span>';
    card.classList.add('correct-flash');
  } else {
    soundWrong();
    opts[chosen].classList.add('wrong');
    fb.className = 'feedback wrong show';
    fb.innerHTML = '❌ <b>Netočno.</b> Točno: <b>' + 'ABCD'[item.a] + '. ' + item.o[item.a] + '</b><span class="explanation">' + item.x + '</span>';
...
function updateMatchBar() {
  ...
  document.getElementById('matchScore').textContent = `✅ ${matchFound}`;
}
...
function lqShowQuestion() {
  ...
  document.getElementById('lqScore').textContent = `✅ ${lqScore}`;
}
...
function lqSelect(el, chosen) {
  ...
  if (chosen === correct) {
    ...
    document.getElementById('lqFeedback').textContent = `❌ Točno: ${correct}`;
